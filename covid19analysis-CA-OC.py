...
df = df[(df['state'] == 'California') & (df['county'] == 'Orange')]
...
plt.title('Orange County COVID-19 New Cases By Day')
plt.xlabel('Date')
...
print('Based on current weekly trends, it would take ' + str(
    fifty) + ' days until the average number of new daily cases in '
                     'Orange County, CA is at 50 or less.')

ten = ceil((10 - b) / m)
print('Based on current weekly trends, it would take ' + str(
    ten) + ' days until the average number of new daily cases in '
                    'Orange County, CA is at 10 or less.')
